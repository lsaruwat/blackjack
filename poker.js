...
function play()
{
	...
	deck1.createDeck();
	deck1.shuffle();
	document.getElementById("replay").addEventListener("click", function(){deck1.dealPoker();}, false);
}
...
function Deck()
{
	...
	this.dealPoker = function deal21()
	{
		...
		var cards = userHand.getElementsByClassName("card");
		console.log(cards);
		for(i=0; i<5; i++)
		{
			cards[i].addEventListener("click", this.hold,false);
		}
	...
	this.hold = function hold(e)
	{
		var card = e.target;
		if(typeof card.className == "null" && card.className != "card") card = card.parentNode;
		card.setAttribute("style", "border: solid 2px red;");
	}
